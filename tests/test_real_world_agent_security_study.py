...
def test_real_world_agent_security_protocol_is_valid() -> None:
    ...
    assert report["candidates"]["pending"] == 257
    assert report["candidates"]["included"] == 56
    assert report["candidates"]["excluded"] == 52
    assert report["candidates"]["reviewed"] == 108
    assert report["candidates"]["prior_study_checks_pending"] == 257
