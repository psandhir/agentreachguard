...
def test_langgraph_mcp_client_get_tools_reconstructs_authority(tmp_path: Path) -> None:
...
""",
    ...
    assert server.metadata["auth_source"] == "authorization-header"
    ...
    identity = next(item for item in graph.all_identities() if item.name == server.identity)
    assert identity.provider == "mcp"
    assert identity.credential_source == "authorization-header"
    assert identity.metadata["runtime_verified"] is False

    adg = graph.adg.as_dict()
    node_kinds = {node["kind"] for node in adg["nodes"]}
    edge_kinds = {edge["kind"] for edge in adg["edges"]}
    assert "mcp_server" in node_kinds
    assert "mcp_tool_filter" in node_kinds
    assert "INVOKES" in edge_kinds
    assert "USES_IDENTITY" in edge_kinds
    assert "CONNECTS_TO" in edge_kinds
    assert "GUARDED_BY" in edge_kinds


def test_filesystem_mcp_resource_authority_is_projected(tmp_path: Path) -> None:
    (tmp_path / "agent.py").write_text(
        """
from langchain_mcp_adapters.client import MultiServerMCPClient
from langgraph.prebuilt import create_react_agent
...
client = MultiServerMCPClient({
    "filesystem": {
        "transport": "stdio",
        "command": "npx",
        "args": ["-y", "@modelcontextprotocol/server-filesystem", "/srv/customer-data"],
    }
})
...
""",
    ...
    assert any(
        edge["kind"] == "READS_FROM" and edge["target"] == resource_id
        for edge in adg["edges"]
    )
    assert any(
        edge["kind"] == "WRITES_TO" and edge["target"] == resource_id
        for edge in adg["edges"]
    )
