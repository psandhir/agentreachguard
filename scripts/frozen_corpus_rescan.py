...
def build_summary(results: list[dict]) -> dict:
    ...
    numeric = [
        ...
        "agent_mapped_flows", "attack_paths", "static_dataflow_attack_paths",
...
def render_markdown(report: dict) -> str:
    ...
    lines = [
        ...
        f"- Flows: {totals['flow_paths']} total / {totals['agent_mapped_flows']} agent-mapped",
        ...
        "## Per repository",
        "",
        "| Repository | Scan | Incomplete | Agents | MCP | Bound MCP | Flows | Mapped | Static paths | Approval controls | Findings | Runtime | Seconds |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for item in report["results"]:
        ...
        lines.append(
            ...
            "{mapped} | {static_paths} | {approval} | {findings} | {runtime} | {seconds} |".format(
                repo=item["repo"],
                scan=item.get("scan"),
                ...
                approval=item.get("approval_control_nodes", "-"),
                findings=item.get("findings", "-"),
                runtime=contexts.get("runtime", 0),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=f"horustrace-cohort-{args.cohort}-") as temp:
        ...
        for index, entry in enumerate(manifest["repositories"], start=1):
            ...
            print(
                ...
                f"mapped={item.get('agent_mapped_flows', '-')} findings={item.get('findings', '-')}",
                flush=True,
            )
    ...
    report = {
        ...
        "scanner_commit": os.environ.get("TARGET_SCANNER_COMMIT", os.environ.get("GITHUB_SHA", "local")),
