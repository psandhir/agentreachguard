...
from horustrace.gcp_iam import GcpIamSnapshotError
...
def _parser() -> argparse.ArgumentParser:
    ...
    scan_parser.add_argument(
        ...
        type=Path,
        help=(
            "Offline Cloud Asset Inventory search-all-iam-policies JSON export "
            "used to enrich already-discovered GCP service-account identities."
        ),
    )
    ...
    graph_parser.add_argument("--gcp-iam-snapshot", type=Path)
    ...
    aibom_parser.add_argument("--gcp-iam-snapshot", type=Path)
    ...
    if args.command in {"graph", "aibom"}:
        try:
            ...
            graph, _ = scan(
                target,
                config=config,
                gcp_iam_snapshot=args.gcp_iam_snapshot,
            )
        except (
            ConfigError,
            GcpIamSnapshotError,
